...
export default function IPSearchPage() {
  ...
  const handleUrlSubmit = async (event) => {
    ...
    setUrlResult(null);
    try {
      const { data } = await axios.get('/api/search/indicator', { params: { value: urlValue, indicator_type: 'url' } });
      setUrlResult(data);
    } catch (err) {
  ...
  const handleBlockIp = async () => {
    ...
    setBlockSuccess('');
    try {
      await axios.post('/api/blocklist', { ip: result.ip });
      setResult((prev) => (prev ? { ...prev, blocked: true } : prev));
      setBlockSuccess('IP added to blocklist.');
    } catch (err) {
          ...
          <section className="bg-[#0d172a] border border-slate-800/70 rounded-3xl p-6 space-y-4 shadow-[0_25px_60px_rgba(8,17,32,0.55)]">
            ...
            <ul className="space-y-3 text-sm text-slate-300">
              ...
              <li>
                ...
                <span className={`ml-2 font-semibold ${result.computed_verdict?.severity === 'High' ? 'text-rose-300' : 'text-slate-100'}`}>
