...
const AlertModal = ({ alert, onClose, onRefresh }) => {
  ...
  const updateStatus = async (nextStatus) => {
    try {
      ...
      if (alert.incident_id) {
        queryClient.invalidateQueries({ queryKey: ['incident-detail', alert.incident_id] });
      }
      onRefresh?.();
      onClose();
    } catch (ackErr) {
  ...
  const incident = alert.incident_context;
          ...
          <div className="mt-5 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-xs uppercase tracking-wide text-emerald-200">Correlated campaign</p>
                <span className={`inline-flex items-center rounded-full px-3 py-1 text-[11px] font-semibold ${
                  incidentStatusStyles[incident.status] || incidentStatusStyles.Active
                }`}
                >
                  {incident.status}
                </span>
              ...
              <p className="text-xs text-slate-300">{incident.summary}</p>
...
const AlertCard = ({ alert, onSelect }) => {
  ...
  return (
    <article className="space-y-2 rounded-2xl border border-slate-800/60 bg-slate-900/40 p-4">
      ...
      {incident && (
        <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-3 text-xs text-slate-300">
          <div className="flex items-center justify-between gap-2">
            <span className="font-semibold text-slate-100">{incident.name}</span>
                  ...
                  <td className="px-6 py-4">
                    {incident ? (
                      <div className="space-y-1 text-xs">
                        <span className="block font-semibold text-slate-100">{incident.name}</span>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-[10px] text-slate-500">{incident.alert_count} alert{incident.alert_count === 1 ? '' : 's'}</span>
                        </div>
                      </div>
                    ) : (
